...
void decrement_total_byte_count(unsigned char length)
{
    if( (ep2_buffer.total_length - length) > ep2_buffer.total_length)
    {
        ...
        //This occurs when no more data is present to read. Reload to get more data
    }
    else
    {
...
unsigned char get_next_byte()
{
    ...
    if(ep2_buffer.current_index == 511 )
    {
        ...
        SYNCDELAY;
        isr_enter -- ;
        ep2_buffer.total_length = 0;
        ep2_buffer.total_length = EP2BCL | (EP2BCH << 8);

        printf("Got data1 in get_next_byte() %02x, length is %04x %02x, buffer len %d",EP2FIFOBUF[6], EP2BCL | (EP2BCH <<8),EP1INCS,ep2_buffer.total_length);
        printf("[ ");
    ...
    else if(ep2_buffer.total_length == 0)
    {
        //Reload the buffer now
        set_breakpoint_3();
        //Rearm the endpoint
        EP2BCL = 0x80; // write once
        ...
        printf("Got data2 in get_next_byte() %02x, length is %04x %02x, buffer len %d",EP2FIFOBUF[6], EP2BCL | (EP2BCH <<8),EP1INCS,ep2_buffer.total_length);
        printf("[ ");
        for(counter =0 ; counter < ep2_buffer.total_length; counter++)
        {
                printf("%02x-%02d, ",EP2FIFOBUF[counter],counter);
